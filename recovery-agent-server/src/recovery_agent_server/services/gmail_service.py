...
def fetch_unread_replies(max_results: int = 10) -> list[dict]:
    ...
    try:
        service = _build_service(creds)
        # Query messages matching recovery reminders/invoices from the last 2 days
        res = (
            service.users()
            .messages()
            .list(
                userId="me",
                q='subject:("Payment Reminder" OR "Invoice #") newer_than:2d',
                maxResults=25,
            )
            .execute()
        )
        ...
        for meta in messages_meta:
            ...
            clean_email = email_match.group(1) if email_match else from_header.strip()
            ...
            clean_body = re.split(r"\r?\n\s*On\s+.+wrote:\s*", body_text, flags=re.IGNORECASE)[0]
            clean_body = re.split(r"\r?\n\s*-+\s*Original Message\s*-+", clean_body, flags=re.IGNORECASE)[0]
            clean_body = clean_body.strip()

            # Skip messages generated by the bot to prevent self-reply loops
            BOT_SIGNATURES = [
